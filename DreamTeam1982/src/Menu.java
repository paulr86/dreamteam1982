...
Scanner entrada = new Scanner (System.in);
	...
	public static void darBajaAsignatura(ArrayList<Alumno>lista) { // Autor: Álvaro Moya Pino
		Scanner entrada = new Scanner(System.in);
		...
		if(buscarAlumno(lista,dni) != -1){
			...
			System.out.println("Alumno no encontrado, por favor, escriba un dni válido: ");
			darBajaAsignatura(lista);
		}
	}
	
	public static int alumnoTieneAsignatura(Alumno alumno, String asignatura) { // Autor: Álvaro Moya Pino
		...
		do {
			...
			case 5:
				do{
					...
					// Pedimos DNI del alumno 
					System.out.println("Introduzca el DNI del alumno a matricular: ");
					dni = entrada.nextLine();
					
					if(existeAlumno(dni, alumnos)) {
						
						Alumno alumnoEncontrado = alumnos.get(buscarAlumno(alumnos, dni));
						...
						try {
							Menu.matricularAlumnos(alumnos, alumnoEncontrado, asignatura); 
						...
						System.out.println("El alumno no existe, introduzca un dni válido");
						mostrarMenu();
					}						
